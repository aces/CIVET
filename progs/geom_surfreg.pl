...
for ($i=0; $i<=$#conf; $i++) {
  ...
  if ($control_mesh_min<=$conf[$i]{control_size} && $control_mesh_max>=$conf[$i]{control_size}){
    ...
    &do_cmd($surf_reg_smart, "-verbose", "-clobber", $keep_blur_cmd, "-source_blur",
            ...
            $old_surface_mapping, $source_obj_blur, $source_field_blur,
            $target_obj_blur, $target_field_blur, $surface_mapping );

    unlink( $source_obj_blur );
    unlink( $target_obj_blur );
    unlink( $source_field_blur );
    unlink( $target_field_blur );
  }
}
